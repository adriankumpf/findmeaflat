...
class WgGesucht {
  ...
  _run(flatType) {
    return got.post(store.urlWgGesucht(flatType), { body: store.searchCriteria(flatType), header: store.header })
...
//      .paginate('.pagination-bottom-wrapper li:last-child a@href')
//      .limit(3)
        ((err, listings) => {
          if (err) reject(err);
          resolve(listings);
  ...
  _findNew(flatType, listings) {
    return listings.filter(o => o.created > store[`timestamp${flatType}`]);
  }

  _updateTimestamp(flatType, newListings) {
    if (newListings.length > 0) {
      store[`timestamp${flatType}`] = Math.max(...(newListings.map(l => l.created)));
    }
    return newListings;
  }
